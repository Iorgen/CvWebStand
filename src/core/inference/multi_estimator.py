...
# Deploy local openvino
from openvino.inference_engine import IENetwork

from core.utils.ie_module import InferenceContext
from core.utils.helper import cut_rois, resize_input
from core.cv.face_detector import FaceDetector
from core.cv.headPos_Estimator import HeadPosEstimator
from core.cv.landmarks_detector import LandmarksDetector
from core.cv.gaze_Estimator import GazeEestimator
...
class ProcessOnFrame:
    ...
    def __init__(self, args):
        ...
        face_detector_net = self.load_model(mode_face_detection)

        # Load Headposition model on Inference Engine
        head_position_net = self.load_model(model_head_position)

        # Load Landmark regressor model on Inference Engine
        landmarks_net = self.load_model(model_landmark_regressor)

        # Load gaze estimation model on IE
        gaze_net = self.load_model(model_gaze)
...
class DriverMonitoring:
    ...
    def process(self, input_stream, output_stream):
        ...
        while input_stream.isOpened():
            ...
            frame = cv2.rotate(frame, cv2.ROTATE_90_COUNTERCLOCKWISE)  # rotate image 90*
            # frame = cv2.rotate(frame, cv2.ROTATE_90_COUNTERCLOCKWISE)  # rotate image 90*
            # frame = cv2.rotate(frame, cv2.ROTATE_90_COUNTERCLOCKWISE)  # rotate image 90*
            ...
            if self.input_crop is not None:
                frame = DriverMonitoring.center_crop(frame, self.input_crop)
    ...
    def run(self, args):
        ...
        """
        # Open Input stream
        input_stream = DriverMonitoring.open_input_stream(args.input)
        ...
        # Writer or CV Window
        output_stream = DriverMonitoring.open_output_stream(args.output, fps, frame_size)
...
def main():
    ...
    driverMonitoring = DriverMonitoring(args)
